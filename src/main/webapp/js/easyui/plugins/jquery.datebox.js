...
(function($) {
	function _1(_2) {
		...
		function _5(_c) {
			...
			_e.unbind(".datebox").bind(
					"click.datebox",
					function(e) {
						if ($(e.target).hasClass("datebox-button-a")) {
							var _f = parseInt($(e.target).attr(
		...
		function _6(_10) {
			...
			for (var i = 0; i < _4.buttons.length; i++) {
				...
				var t = $(
						...
						.html($.isFunction(btn.text) ? btn.text(_10) : btn.text)
						.appendTo(td);
	...
	$.fn.datebox.parseOptions = function(_2f) {
		return $.extend({}, $.fn.combo.parseOptions(_2f), $.parser
				.parseOptions(_2f, [ "sharedCalendar" ]));
	};
